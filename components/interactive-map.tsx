...
function MapViewport({ located, selected }: { located: boolean; selected: Station | null }) {
  const map = useMap()
  useEffect(() => {
    if (located) map.flyTo([36.8065, 10.1815], 12, { duration: 0.8 })
  }, [located, map])
  useEffect(() => {
...
export function InteractiveMap({ stations, selected, located, onSelect }: { stations: Station[]; selected: Station | null; located: boolean; onSelect: (station: Station) => void }) {
  return (
    <MapContainer center={[36.8065, 10.1815]} zoom={11} zoomControl={false} className="absolute inset-0 z-0" aria-label="Carte interactive des stations-service du Grand Tunis">
      ...
      <MapViewport located={located} selected={selected} />
